/*******************************************************
* ESITapTradeApiSimpleDemoVC2015_X64
* www.xfinapi.com
*******************************************************/
...
2.拷贝XTA_W64\Api\ESITap_vX.X.X.X_XXXXXXXX目录（X.X.X.X_XXXXXXXX为版本号）下的所有dll文件到可执行文件目录，
...
{
	printf("  Tick,%s %s %s, HighestPrice=%g, LowestPrice=%g, BidPrice0=%g, BidVolume0=%lld, AskPrice0=%g, AskVolume0=%lld, LastPrice=%g, LastVolume=%lld, TradingTime=%s\n",
...
{
	printf("  ProductType=%d, Ref=%s, ID=%s, InstID=%s %s %s, Price=%g, Volume=%lld, NoTradedVolume=%lld, Direction=%d, OpenCloseType=%d, PriceCond=%d, TimeCond=%d, VolumeCond=%d, Status=%d, Msg=%s, %s\n",
		order.ProductType,
...
{
	printf("  ID=%s, OrderID=%s, InstID=%s %s %s, Price=%g, Volume=%lld, Direction=%d, OpenCloseType=%d, %s\n",
...
{
	printf("  InstID=%s %s %s, BuyPosition=%lld, SellPosition=%lld, NetPosition=%lld\n",
...
void MarketTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	market = XFinApi_CreateMarketApi("XTA_W64/Api/ESITap_v9.3.0.1_20160406/XFinApi.ESITapTradeApid.dll", &err);
#else
	market = XFinApi_CreateMarketApi("XTA_W64/Api/ESITap_v9.3.0.1_20160406/XFinApi.ESITapTradeApi.dll", &err);
#endif
...
void TradeTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	trade = XFinApi_CreateTradeApi("XTA_W64/Api/ESITap_v9.3.0.1_20160406/XFinApi.ESITapTradeApid.dll", &err);
#else
	trade = XFinApi_CreateTradeApi("XTA_W64/Api/ESITap_v9.3.0.1_20160406/XFinApi.ESITapTradeApi.dll", &err);
#endif
	if (err || !trade)
	{
